...
[X] But the actual stored items and variables are now correct while in-game
[X] But the data does not get saved on relogs...
...
[] Make the chest unbrekable when there are items inside. Don't know how to make it work.
"KillMultiTile" is ran after the tile is destroyed, I need to check before it is destroyed.
Blocks below the chest are unbreakable as intended
(I don't think it works to set Modtile.MinPick, it would change all shimmer chests)

[X] I made the chest drop all items inside when broken
(annoying solution, have to split stacks to 9999, and can in theory go above item limit and lose items).
